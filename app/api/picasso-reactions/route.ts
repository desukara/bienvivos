...
type Target = 'picasso-opening' | 'picasso-colour' | 'picasso-rhythm' | 'picasso-finale';
...
const reconstructedBaseline: Record<Target, Record<ReactionType, number>> = {
  'picasso-opening': { like: 62, love: 40, wow: 24 },
  'picasso-colour': { like: 43, love: 27, wow: 15 },
  'picasso-rhythm': { like: 38, love: 24, wow: 21 },
  'picasso-finale': { like: 55, love: 37, wow: 18 },
};
...
function countKey(target: Target, reaction: ReactionType) { return `eastokyo:picasso:v1:${namespace}:${target}:${reaction}:count`; }
function votersKey(target: Target, reaction: ReactionType) { return `eastokyo:picasso:v1:${namespace}:${target}:${reaction}:voters`; }
...
export async function GET(request: NextRequest) {
  ...
  catch (error) { console.error('Picasso reaction read failed', error); return NextResponse.json({ error: 'Reaction store unavailable', persistent: false }, { status: 503 }); }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('Picasso reaction update failed', error);
