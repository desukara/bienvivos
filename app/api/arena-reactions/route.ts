...
type Target = 'arena-article' | 'arena-drama' | 'arena-return';
...
const reconstructedBaseline: Record<Target, Record<ReactionType, number>> = {
  'arena-article': { like: 56, love: 31, wow: 18 },
  'arena-drama': { like: 39, love: 21, wow: 21 },
  'arena-return': { like: 37, love: 24, wow: 14 },
};
...
function countKey(target: Target, reaction: ReactionType) { return `eastokyo:arena:v1:${namespace}:${target}:${reaction}:count`; }
function votersKey(target: Target, reaction: ReactionType) { return `eastokyo:arena:v1:${namespace}:${target}:${reaction}:voters`; }
...
export async function GET(request: NextRequest) {
  ...
  catch (error) { console.error('Arena reaction read failed', error); return NextResponse.json({ error: 'Reaction store unavailable', persistent: false }, { status: 503 }); }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('Arena reaction update failed', error);
